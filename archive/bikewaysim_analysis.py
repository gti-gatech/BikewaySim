...
def map_trips(links, origs, dests, paths):
    ...
    trip_lines = paths_geo.dissolve(by='trip_id').reset_index()[['trip_id','geometry']]
    
    return trip_lines

#%%

trip_lines = map_trips(links, origs, dests, paths_bike)

trip_lines.to_file(user_directory+r'\Documents\BikewaySimData\tu_delft\time_trip_lines.geojson',driver='GeoJSON')


#%% redo with perceived

#network files
network_dir = user_directory + r'\Documents\BikewaySimData\processed_shapefiles\prepared_network\per_dist'
links = gpd.read_file(network_dir+r'\links\links.geojson')[['A','B','geometry']]
...
per_trip_lines = map_trips(links, origs, dests, paths_bike)

per_trip_lines.to_file(user_directory+r'\Documents\BikewaySimData\tu_delft\imped_trip_lines.geojson',driver='GeoJSON')

#%%

trip_lines = trip_lines.reset_index()
trip_lines['imp'] = 'dist'

per_trip_lines = per_trip_lines.reset_index()
per_trip_lines['imp'] = 'per_dist'

combine = trip_lines.append(per_trip_lines)

selection = combine[combine['trip_id'] == '538_630']

selection.to_file(user_directory+r'\Documents\BikewaySimData\tu_delft\routing.geojson', driver = 'GeoJSON')
