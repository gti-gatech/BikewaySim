...
def suggested_matches(cycleways_osm,other_source,other_name,buffer_ft,max_hausdorff_dist):
    ...
    overlap.loc[overlap.drop(columns=[f"{other_name}_id",'geometry']).duplicated()] = False

    ### STREET INFRA MATCHED TO MULTI USE PATH CHECK ###
    street_infra = ['bike lane','buffered bike lane','sharrow','cycletrack']
    ...
    overlap.loc[osm_is_street & other_is_mup,'accept_match'] = False
    ...
    overlap.loc[osm_is_mup & other_is_street,'accept_match'] = False
    ...
    osm_is_street = overlap[["facility_fwd",'facility_rev']].isin(street_infra).any(axis=1)
    other_is_street = overlap[f"{other_name}_osm_type"].isin(street_infra)
    overlap.loc[osm_is_street & other_is_street & (overlap['name_check']==False),'accept_match'] = False
    ...
    overlap['one_to_many'] = overlap.groupby('osmid').transform(lambda x: x.isna().sum() > 1)
    ...
    overlap.loc[
        overlap['name_check'] & (overlap['one_to_many']==False) & overlap['accept_match'].isna(),'accept_match'] = True
    
    ### MULTI USE PATH ONE TO ONE ###
    # accept if both are multi-use paths and it's a one to one match
    overlap.loc[
        osm_is_mup & other_is_mup & (overlap['one_to_many']==False) & overlap['accept_match'].isna(),'accept_match'] = True
    ...
    # for accept_match == NULL set it equal to match with the minimum
    ...
    overlap.loc[overlap['accept_match'].isna(),'accept_match'] = np.nan
