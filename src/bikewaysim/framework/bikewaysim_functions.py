...
def snap_ods_to_network(od_gdf:pd.DataFrame,nodes:gpd.GeoDataFrame,od_list=False):
    ...
    closest_node.rename(columns={'od_geometry':'geometry'},inplace=True)
    closest_node.set_geometry('geometry',inplace=True)

    return closest_node

def weighted_impedance_reduction(od_matrix, travel_time_dict, current_results, future_results, shortest_results):
    '''
    Retrieves the travel time and impedance for the shortest path and impedance routing on the current and future network
    '''

    # get the travel times for each run
    current_tt = {key: np.sum([travel_time_dict.get(edge[0]) for edge in item['edge_list']]) for key, item in current_results.items()}
    future_tt = {key: np.sum([travel_time_dict.get(edge[0]) for edge in item['edge_list']]) for key, item in future_results.items()}
    shortest_tt = {key: np.sum([travel_time_dict.get(edge[0]) for edge in item['edge_list']]) for key, item in shortest_results.items()}

    # assign travel times to trips in od_matrix
    od_matrix['current_tt_min'] = [current_tt.get(x) for x in list((zip(od_matrix['orig_N'],od_matrix['dest_N'])))]
    od_matrix['future_tt_min'] = [future_tt.get(x) for x in list((zip(od_matrix['orig_N'],od_matrix['dest_N'])))]
    od_matrix['shortest_tt_min'] = [shortest_tt.get(x) for x in list((zip(od_matrix['orig_N'],od_matrix['dest_N'])))]

    # retrieve the trip impedance
    current_impedance = {key:item['length'] for key, item in current_results.items()}
    future_impedance = {key:item['length'] for key, item in future_results.items()}

    # assign trip impedance to trips in od_matrix
    od_matrix['current_impedance'] = [current_impedance.get(x) for x in list((zip(od_matrix['orig_N'],od_matrix['dest_N'])))]
    od_matrix['future_impedance'] = [future_impedance.get(x) for x in list((zip(od_matrix['orig_N'],od_matrix['dest_N'])))]

    # get the weighted impedance reduction
    ...
    print('The impedance for',(od_matrix['impedance_reduction']==0).sum(),'OD pairs did not decrease')
    print(f"The max reduction in impedance was {od_matrix['impedance_reduction'].max():.2f} minutes")
    return od_matrix
