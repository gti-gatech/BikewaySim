...
suffixes = {
    ...
    'northwest': 'nw',
    'southwest': 'sw',
    'martin luther king': 'mlk' # this wont work with the current method of using split
    # Add more suffixes and their abbreviations as needed
}

def contract_suffix(street_name):
    ...
    new_word = []
    for x in words:
        if x in suffixes.keys():
            new_word.append(suffixes[x])
        else:
            new_word.append(x)
...
def name_check(name1,name2,accept_ratio=0.7):
    '''
    Use's difflib's sequence matcher to find the similarity between two street names.
    After examining some results, 0.8 was around where you would see street names that
    should probably match but setting in case the suffixes are different.
    This method helps avoid issues with typos.
    '''
